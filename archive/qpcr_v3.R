...
analysis_method = tk_select.list(choices = c("Comparative ct (multiple hk)",
                                             "Comparative ct (2^-ddct)",
                                             "Standard curve"),
                                       preselect = "Comparative ct (multiple hk)",
                                       multiple = FALSE,
                                       title = paste0("Select qPCR analysis method\n
                                       The 'Multiple housekeeping' method uses the formula '2^dctGOI / geomean(2^dctREFS)'.\n
...
if (analysis_method == "Standard curve") {
  ...
  vol_var = tk_select.list(choices = names(settings),
                           multiple = FALSE,
                           title = "Select sampel volume column")
...
nc_samples = tk_select.list(choices = c(NA, unique(settings[[unique_id]])),
                            preselect = NA,
                            multiple = TRUE,
                            title = "Select negative control sample/s\n
                              (Samples you expect to be empty.\n
                              Used to exclude housekeeping genes if they fail in a real sample)")
...
#===============================================================================
# Select targets as GOI, HK (or IPC if included)
#===============================================================================
if (grepl("Comparative", analysis_method, fixed = TRUE)) {
...
# Internal positive control target
...
#===============================================================================
# Find outlier internal positive control (IPC) wells
#===============================================================================
...
if (grepl("Comparative", analysis_method, fixed = TRUE)) {
  ...
  # Target summary
  targets <- rbindlist(lapply(ct, extract_sublist, sub = "summary")) %>%
    select(-c(starts_with("n_"), sd)) %>%
    dplyr::mutate(sample_type = ifelse(!!sym(unique_id) %in% nc_samples, "nc", "real")) %>%
...
# I'M HERE
#===============================================================================
if (analysis_method == "Comparative ct (multiple hk)") {
  
  source("./functions/multiple_hk.R")
  comparative_ct <- multiple_hk(data = ct)
  
} else if (analysis_method == "Comparative ct (2^-ddct)") {
  
  source("./functions/twoddct.R")
  comparative_ct <- twoddct(data = ct)
