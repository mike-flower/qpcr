...
# Create named vector of grouping variables
if (is.na(group_n)) {
  ...
  for (n in seq(as.numeric(group_n))) {
    ...
    var <- tk_select.list(choices = names(settings),
                          ...
                          title = paste0("Select grouping variable ", n, "\n
                                       1 = x-axis, 2 = facet columns, 3 = facet rows"))
...
if (analysis_method == "Standard curve") {
  ...
  # Dependent variables vector for stats
  # dependent_vector <- c("Concentration" = "predict_conc",
  #                       "DNA yield" = "total_DNA")
...
# qPCR stats
#===============================================================================
source("./functions/qpcr_stats 2.0.R")
qpcr_stats(quantification = quantification,
           groups = grouping_vector,
           dependent_vars = dependent_vector,
           y_log = y_log) # I'M HERE - NEED TO ADD THIS AS A VARIABLE IN THE STATS SECTION
