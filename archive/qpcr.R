...
removeSheet(wb, sheetName = "raw")
saveWorkbook(wb, file.path(out.dir, "qpcr.xlsx"))
write.xlsx(ct,
           file = file.path(out.dir, "qpcr.xlsx"),
           sheetName = "raw", append = T, row.names = F)
...
# ct$target[!is.na(ct$target)] <-sapply(ct$target[!is.na(ct$target)],
#                                       janitor::make_clean_names,
#                                       USE.NAMES = F, replace = c(" " = "_")) # https://stackoverflow.com/questions/62594765/keep-duplicates-using-make-clean-names-in-r-janitor-package
...
exclude_failed_hk = tk_select.list(choices = c("Yes", "No"),
                                 preselect = "Yes",
                                 multiple = FALSE,
                                 title = "Exclude housekeeper target when ≥1 sample fails")
...
s <- settings %>%
  ...
  #        -starts_with(c("reporter", "target"))) %>%
  select(any_of(c(unique_id, xvar, facet_col, facet_row))) %>%
  distinct() %>%
...
#===============================================================================
relative_expression <- comparative_ct %>%
  select(any_of(names(s)), starts_with("relative_expression"))
names(relative_expression) <- gsub("relative_expression_", "", names(relative_expression))

# Export relative expression
wb = loadWorkbook(file.path(out.dir, "qpcr.xlsx"))
removeSheet(wb, sheetName = "relative_expression")
saveWorkbook(wb, file.path(out.dir, "qpcr.xlsx"))
write.xlsx(as.data.frame(relative_expression), file = file.path(out.dir, "qpcr.xlsx"),
...
plots <- lapply(setNames(goi, goi), function(g) {
  ...
  # REMOVE NA FROM RELATIVE_EXPRESSION AND CALL IT R
  r <- relative_expression %>%
  ...
  if(sum(!is.na(r[[g]])) > 0) {
    ...
    rep_count <- r %>%
      group_by(!!sym(xvar), !!sym(facet_col), !!sym(facet_row)) %>%
      dplyr::summarise(n = n())
    rep_count
    
    if (length(unique(filter(rep_count, n > 1)[[xvar]]))) {
      
      # t-test
      ttest <- r %>%
        left_join(rep_count, by = c(xvar, facet_col, facet_row)) %>%
        filter(n > 1) %>%
        group_by(!!sym(facet_col), !!sym(facet_row)) %>%
        t_test(as.formula(paste0(g, " ~ ", xvar))) %>%
        add_y_position()
      ttest
      
    } else if (facet_col != "" && facet_row != "") {
      
      ttest <- data.frame(.y. = g, group1 = NA, group2 = NA, n1 = NA, n2 = NA,
                          statistic = NA, df = NA, p = NA, y.position = NA, groups = NA) %>%
        dplyr::mutate(!!sym(facet_col) := as.character(r[[facet_col]][1]),
                      !!sym(facet_row) := as.character(r[[facet_row]][1])) %>%
        relocate(any_of(c(facet_col, facet_row)))
      ttest
      
    }
    
    # Boxplot with ttest
    ggplot(r, aes(x = !!sym(xvar), y = !!sym(g))) +
