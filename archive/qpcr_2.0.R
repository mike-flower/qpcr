...
if (default) {
  ...
  # Group variables
  group_var <- tk_select.list(choices = names(settings),
                              preselect = "group",
                              multiple = FALSE,
                              title = "Select grouping column")
  
  group_label <- dlgInput(message = "Enter group label for plots",
                          default = str_to_sentence(group_var))$res
  
  group_rename <- eval(parse(text = dlgInput(message = "Enter named character vector to rename groups
                        e.g. c(\"original_name\"=\"New name\")",
                                             default = "NA")$res))
