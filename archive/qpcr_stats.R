...
rm(list = lnames[!lnames %in% c("comparative_ct", "ct", "goi", "calibrators",
...
s <- settings %>%
  select(any_of(c(unique_id, group1))) %>%
  distinct()
...
#===============================================================================
# Convert comparative ct table into a list by GOI target
#===============================================================================
relative_expression <- relative_expression %>%
  # pivot_longer(cols = any_of(c(goi, "ARSE")),
...
# T-test each target
plot <- lapply(relative_expression, function(x) {
