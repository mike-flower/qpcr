...
plot <- lapply(ct, function(x) {
  ...
  plot <-
    ...
    labs(title = "Technical replicate outliers",
         subtitle = t,
         x = unique_id_label,
         y = "Threshold cycle (ct)",
         fill = "Outlier") +
...
if (analysis_method == "Comparative ct (multiple housekeepers)") {
  ...
  source("./functions/comparative_prep_2.2.R")
  ...
  source("./functions/multiple_hk_2.1.R")
  ...
  plots <- lapply(setNames(goi, goi), function(g) {
    ...
    plot <-
      ...
      geom_bar(aes(fill = !!sym(calibrator_group_var), colour = !!sym(calibrator_var)), stat = "identity") +
...
# qPCR stats
#===============================================================================
source("./functions/qpcr_stats_2.3.R")
