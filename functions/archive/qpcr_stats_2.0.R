...
# qPCR stats
#===============================================================================
qpcr_stats <- function(quantification, group_var, group_label, group_rename,
                       dependent_vars, y_log, hide_ns) {
  ...
  #===============================================================================
  s <- settings %>%
    select(any_of(c(unique_id, group_var))) %>%
    distinct()
  ...
  quantification_clean <- lapply(quantification, function(x) {
    ...
    # Remove NA from grouping variable and add factor levels
    y <- y %>%
      filter(!is.na(!!sym(group_var))) %>%
      dplyr::mutate(!!sym(group_var) := factor(!!sym(group_var), levels = unique(settings[[group_var]]))) %>%
      droplevels()
