...
qpcr_stats <- function(quantification, grouping_vars, dependent_vars, y_log, hide_ns) {
  ...
  ttest_plot <- lapply(setNames(names(dependent_vars), names(dependent_vars)), function(d) {
    ...
    ttest_plot <- lapply(quantification_clean, function(x) {
      ...
      # Only run if there is quantification data
      if (nrow(x) > 0) {
        ...
        # Filter for groups with >1 sample
        y <- x %>%
          left_join(group_count, by = unname(grouping_vars)) %>%
          filter(n_samples > 1)
        
        # Count unique variables in each group (can't do groupwise stats if only one variable in a group)
        n_vars <- lapply(grouping_vars, function(g) {
          return(length(unique(y[[g]])))
        })
        n_vars <- unname(unlist(n_vars))
        
        # Filter for groups with >1 variable
        grouping_vars_clean <- data.frame(name = names(grouping_vars),
                                          value = unname(grouping_vars),
                                          n_vars = n_vars) %>%
          dplyr::filter(n_vars > 1)
        grouping_vars_clean <- as.vector(setNames(grouping_vars_clean$value, grouping_vars_clean$name))
        
        # Recount samples in each group using the new grouping_var_clean
        group_count <- y %>%
          group_by_at(unname(grouping_vars_clean)) %>%
          dplyr::summarise(n_samples = n())
        ...
        if(nrow(group_count %>% filter(n > 1)) > 1) {
          ...
          ttest <- y %>%
            left_join(group_count, by = unname(grouping_vars_clean)) %>%
            filter(n > 1) %>%
            ...
            group_by_at(unname(grouping_vars_clean)[-1]) %>% # First grouping variable is used as independent variable in t-test
            t_test(as.formula(paste0(dependent_vars[[d]], " ~ ", unname(grouping_vars_clean)[1]))) %>%
            add_y_position()
          ...
          # Facet formula
          if (length(grouping_vars_clean) > 1) {
            facet_formula <- as.formula(paste(unname(grouping_vars_clean)[-1], collapse = " ~ "))
          } else {
            facet_formula <- NA
          }
          facet_formula
          ...
          # Plot
          p <-
            ggplot(x, aes(x=!!sym(grouping_vars_clean[[1]]), y=!!sym(dependent_vars[[d]]))) +
  ...
  for (d in names(plots)) {
    ...
    if (length(plots[[d]][!is.na(plots[[d]])]) > 0) {
      print(
      ...
      ggsave(file.path(out_dir, "ttest.png"))
