...
standard_curve <- function(ct) {
  ...
  # Export r2 table
  wb = loadWorkbook(file.path(out_dir, "qpcr.xlsx"))
  removeSheet(wb, sheetName = "r2")
  saveWorkbook(wb, file.path(out_dir, "qpcr.xlsx"))
  rm(wb)
  write.xlsx(r2,
  ...
  if (quantifiler == "Yes") {
    ...
    quality <- rbindlist(y) %>%
      select(filename, well_short, !!sym(unique_id), sample_type, tech_outlier, target, predict_conc) %>%
  ...
  print(
    wrap_plots(plot) +
      plot_annotation(title = "Concentration",
                      theme = theme(plot.title = element_text(hjust = 0.5, face = "bold")))
  )
  ...
  print(
    wrap_plots(plot) +
      plot_annotation(title = "DNA yield",
                      theme = theme(plot.title = element_text(hjust = 0.5, face = "bold")))
  )
