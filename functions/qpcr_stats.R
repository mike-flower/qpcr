...
qpcr_stats <- function(quantification, grouping_vars, dependent_vars, y_log, hide_ns) {
  ...
  s <- settings %>%
    ...
    select(any_of(c(unique_id, grouping_vars$var))) %>%
    distinct()
  ...
  ttest_plot <- lapply(setNames(names(dependent_vars), names(dependent_vars)), function(d) {
    ...
    ttest_plot <- lapply(quantification_clean, function(x) {
      ...
      facet_row <- grouping_vars$var[match("facet rows", grouping_vars$loc)]
      facet_col <- grouping_vars$var[match("facet columns", grouping_vars$loc)]
      
      # Convert NA to .
      for (l in c("x_axis", "facet_row", "facet_col")) {
        m <- ifelse(is.na(get(l)), ".", get(l))
        assign(l, m)
      }
      rm(l, m)
      
      # Facet formula
      if (nrow(grouping_vars) == 1) {
        facet_formula <- NA
      } else if (nrow(grouping_vars) > 1) {
        facet_formula <- as.formula(as.formula(paste0(facet_row, " ~ ", facet_col)))
      }
      facet_formula
      ...
      # Plot
      p <-
        ggplot(x, aes(x=!!sym(x_axis), y=!!sym(dependent_vars[[d]]))) +
          ...
          labs(title = t,
               x = grouping_vars$label[match(x_axis, grouping_vars$var)],
               y = d) +
